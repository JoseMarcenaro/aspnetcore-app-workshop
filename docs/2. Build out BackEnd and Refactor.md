...
* Merge commit `Session #2, Breakpoint #2`
...
1. Use the Swagger UI to upload [NDC_London_2020.json](/src/BackEnd/Data/Import/NDC_London_2020.json) to the `/api/Sessions/upload` API, using the `Sessionize` format (not "--")
...
### Code
* Merge commit `Session #2, Breakpoint #3`

### Database

1. Delete the database `aspnetcore-workshop3p-backend` 
2. In OneDrive settings, add the folder "netcore" for download
3. Import bacpac: `aspnetcore-workshop3p-backend-session2` as **`aspnetcore-workshop3p-backend`** database

### Code Review
* (data loaders) disregard, for now
* SpeakerController.GetSpeakers( ) changes
* SpeakerController.GetSpeaker( ) changes
* MapSpeakerResponse( ) extension method
* Entity classes (Speaker, SessionSpeaker) => Data classes (Speaker) => VM classes (SpeakerResponse)
